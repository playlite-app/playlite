...
/// Resultado do callback OAuth capturado no servidor local.
...
/// Gera um valor aleatório para o parâmetro `state` (proteção CSRF).
...
pub fn wait_for_auth_code(port: u16) -> Result<AuthCallbackResult, String> {
    let address = format!("127.0.0.1:{}", port);
    // Tenta iniciar o servidor
    let server =
        Server::http(&address).map_err(|e| format!("Porta ocupada ou erro de rede: {}", e))?;

    let (tx, rx) = mpsc::channel::<AuthCallbackResult>();
